...
START_TEST(truncate_test_1) {
  ...
  for (size_t i = 0; i < sizeof(values) / sizeof(my_decimal); i++) {
    my_decimal res;
    int ret = my_truncate(values[i], &res);
...
START_TEST(truncate_test_2) {
  my_decimal error_value_1 = {
      {0x0000000A, 0x0000000D, 0x0000033B, 0x001E0000}};
  my_decimal error_value_2;
  int ret = my_truncate(error_value_1, &error_value_2);
...
Suite* my_truncate_suite(void) {
  Suite* s;
  s = suite_create("\033[33mmy_truncate\033[0m");

  TCase* tc_core;
  tc_core = tcase_create("my_truncate_core");
