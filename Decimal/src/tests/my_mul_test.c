...
START_TEST(mul_test_1) {
  ...
  for (size_t i = 0; i < sizeof(values_1) / sizeof(my_decimal); i++) {
    my_decimal res;
    int ret = my_mul(values_1[i], values_2[i], &res);
...
START_TEST(mul_test_2) {
  ...
  for (size_t i = 0; i < sizeof(error_values_1) / sizeof(my_decimal); i++) {
    my_decimal res;
    int ret = my_mul(error_values_1[i], error_values_2[i], &res);
...
Suite* my_mul_suite(void) {
  Suite* s;
  s = suite_create("\033[33mmy_mul\033[0m");

  TCase* tc_core;
  tc_core = tcase_create("my_mul_core");
