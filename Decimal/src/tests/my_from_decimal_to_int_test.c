...
START_TEST(from_decimal_to_int_test_1) {
  for (size_t i = 0, j = 0; i < sizeof(src) / sizeof(my_decimal); i++) {
    int convert;
    int res = my_from_decimal_to_int(src[i], &convert);
    if (res == FALSE) {
...
Suite* my_from_decimal_to_int_suite(void) {
  Suite* s;
  s = suite_create("\033[33mmy_from_decimal_to_int\033[0m");

  TCase* tc_core;
  tc_core = tcase_create("my_from_decimal_to_int_core");
