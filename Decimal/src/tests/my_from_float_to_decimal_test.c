...
START_TEST(from_float_to_decimal_test_1) {
  ...
  ck_assert_int_eq(ERROR, my_from_float_to_decimal(float_1, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(null.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(ERROR, my_from_float_to_decimal(float_2, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(null.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(ERROR, my_from_float_to_decimal(float_3, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(null.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(ERROR, my_from_float_to_decimal(float_4, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(null.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(ERROR, my_from_float_to_decimal(float_5, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(null.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(ERROR, my_from_float_to_decimal(float_6, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(null.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(ERROR, my_from_float_to_decimal(float_7, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(null.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(ERROR, my_from_float_to_decimal(float_8, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(null.bits[i], result.bits[i]);
  }
}
...
START_TEST(from_float_to_decimal_test_2) {
  ...
  ck_assert_int_eq(OK, my_from_float_to_decimal(float_1, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_1.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_float_to_decimal(float_2, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_2.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_float_to_decimal(float_3, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_3.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_float_to_decimal(float_4, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_4.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_float_to_decimal(float_5, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_5.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_float_to_decimal(float_6, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_6.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_float_to_decimal(float_7, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_7.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_float_to_decimal(float_8, &result));
  for (int i = 0; i < 4; i++) {
...
START_TEST(from_float_to_decimal_test_5) {
  ...
  my_decimal value_1 = {{0x0012D688, 0x00000000, 0x00000000,
                          0x00000000}};  //                       1234568
  my_decimal value_2 = {{0x00000014, 0x00000000, 0x00000000,
                          0x00000000}};  //                            20
  my_decimal value_3 = {{0x008CE89C, 0x00000000, 0x00000000,
                          0x80070000}};  //                    -0.9234588
  my_decimal value_4 = {{0x000000A7, 0x00000000, 0x00000000,
                          0x80050000}};  //                      -0.00167
  my_decimal value_5 = {{0xA9400000, 0xB25EFECA, 0xE2BD0685,
...
Suite* my_from_float_to_decimal_suite(void) {
  Suite* s;
  s = suite_create("\033[33mmy_from_float_to_decimal\033[0m");

  TCase* tc_core;
  tc_core = tcase_create("my_from_float_to_decimal_core");
