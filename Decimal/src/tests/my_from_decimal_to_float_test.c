...
START_TEST(from_decimal_to_float_test_1) {
  for (size_t i = 0; i < sizeof(src) / sizeof(my_decimal); i++) {
    float convert;
    int res = my_from_decimal_to_float(src[i], &convert);
...
Suite* my_from_decimal_to_float_suite(void) {
  Suite* s;
  s = suite_create("\033[33mmy_from_decimal_to_float\033[0m");

  TCase* tc_core;
  tc_core = tcase_create("my_from_decimal_to_float_core");
