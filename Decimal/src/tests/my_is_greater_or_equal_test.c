...
START_TEST(is_greater_or_equal_test_1) {
  ...
  for (size_t i = 0; i < sizeof(values_1) / sizeof(my_decimal); i++) {
    ck_assert_int_eq(TRUE, my_is_greater_or_equal(values_1[i], values_2[i]));
  }
}
END_TEST
...
START_TEST(is_greater_or_equal_test_2) {
  ...
  for (size_t i = 0; i < sizeof(values_1) / sizeof(my_decimal); i++) {
    ck_assert_int_eq(FALSE, my_is_greater_or_equal(values_1[i], values_2[i]));
  }
}
END_TEST
...
Suite* my_is_greater_or_equal_suite(void) {
  Suite* s;
  s = suite_create("\033[33mmy_is_greater_or_equal\033[0m");

  TCase* tc_core;
  tc_core = tcase_create("my_is_greater_or_equal_core");
