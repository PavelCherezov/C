...
START_TEST(from_int_to_decimal_test_1) {
  ...
  ck_assert_int_eq(OK, my_from_int_to_decimal(int_1, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_1.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_int_to_decimal(int_2, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_2.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_int_to_decimal(int_3, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_3.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_int_to_decimal(int_4, &result));
  for (int i = 0; i < 4; i++) {
    ck_assert_int_eq(value_4.bits[i], result.bits[i]);
  }
  ck_assert_int_eq(OK, my_from_int_to_decimal(int_5, &result));
  for (int i = 0; i < 4; i++) {
...
Suite* my_from_int_to_decimal_suite(void) {
  Suite* s;
  s = suite_create("\033[33mmy_from_int_to_decimal\033[0m");

  TCase* tc_core;
  tc_core = tcase_create("my_from_int_to_decimal_core");
